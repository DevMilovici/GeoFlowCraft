...
def read_sentinel2_bands(data_path, from_aws=False, bucket='sentinel-s2-l2a', channels_last=False):
    ...
    #if the zip file contains the subdirectories R10m, R20m and R60m - default contains no subdir as latest zips from 2023
    subdir = False
    archive = None
    print(data_path)
    ...
    for res in bands_dir.keys():
        ...
        for i in range(len(bands_dir[res]['band_names'])):
            ...
            if from_aws:
                ...
                #     if name.endswith('IMG_DATA/R10m/'):
                #         subdir = True
                #         break
                ...
                # else:
                #     path_img_data = [name for name in archive.namelist() if name.endswith('{}.jp2'.format(band_name))][0]
                
                if archive is None:
                    #The bands are already unzipped, e.g., Delta S2 series from Dana (the path is absolute)
                    path_img_data = [name for name in listdir(data_path) if name.endswith('{}.tif'.format(band_name))][0]
    ...
    #I have to get this band for each sample in series (Dana)
    if from_aws:
        path_band = os.path.join('/vsis3', bucket, data_path, 'qi', 'CLD_20m.jp2')
    elif archive is None:
        path_img_data = [name for name in listdir(data_path) if name.endswith('CLD_20m.jp2') or name.endswith('MSK_CLDPRB_20m.jp2') or name.endswith('MSK_CLDPRB.tif')][0]
        path_band = os.path.join(data_path, path_img_data)
    else:
        path_img_data = \
        [name for name in archive.namelist() if name.endswith('CLD_20m.jp2') or name.endswith('MSK_CLDPRB_20m.jp2') or name.endswith('MSK_CLDPRB.tif')][0]
        path_band = os.path.join(data_path, path_img_data)
        path_band = '/vsizip/' + path_band
    ...
    interpolation_map = {
        0: cv2.INTER_NEAREST,   #calitate:scăzută, viteza:foarte rapida
        3: cv2.INTER_CUBIC      #calitate:buna, viteza:mai lenta
    }

    for band_name, band_array in band_arrays.items():
        if band_array.shape != target_shape:
            if band_name == 'SCL':
                interp = interpolation_map[0]
            else:
                interp = interpolation_map[3]
            start1 = time.time()
            resized = cv2.resize(
                band_array,           # input
            ...
            band_arrays[band_name] = resized.astype(np.uint16)
            end1 = time.time()
            print("TIME RESIZING FOR ", band_name, ":", time.strftime('%H:%M:%S', time.gmtime(end1 - start1)))
            
    end = time.time()
    print("TIME RESIZING:", time.strftime('%H:%M:%S', time.gmtime(end - start)))

    print('sorting bands...')
    start = time.time()
    image_array = sort_band_arrays(band_arrays=band_arrays, channels_last=channels_last)
    end = time.time()
    print("TIME SORTING:", time.strftime('%H:%M:%S', time.gmtime(end - start)))
...
def get_reference_band_path(path_zip_file, ref_band_suffix='B02_10m.jp2'):
    ...
    #refDataset_path = os.path.join(path_zip_file, archive_B02)
...
def get_reference_band_ds_gdal(path_file, ref_band_suffix='B02_10m.jp2'):
    if ".zip" in path_file:
        ...
        # create path on aws s3 - we do not use aws s3 as source data
        #refDataset_path = os.path.join('/vsis3', 'sentinel-s2-l2a', path_file, 'R10m', 'B02.jp2')
        refDataset_path = os.path.join(path_file, 'B02.tif')
